...
## 2026-06-08T06:03:29Z

Added CTF raw capture mode. Setting `CAPTURE_REDACT=false` now disables server-side redaction for captured headers, JSON bodies, text previews, CLI reports, fingerprint requests, and proxy response summaries. Companion supports `--no-redact` or `RT_REFRESH_REDACT=false` to upload raw reports, while `--include-raw` remains available for raw file text. UI and README now document raw mode. Validation passed: `npm test` 10/10, `node --check public/app.js`, `node --check src/server.js`, `node --check src/cpa.js`, `node --check scripts/cli-companion.mjs`, `docker compose config`, local raw runtime smoke showing original `x-test-token`, JSON `refresh_token`, and response auth values, and pulled-GHCR raw container smoke. Pushed commit `99c8b9a` and multi-arch GHCR tags `latest` and `99c8b9a`; verified amd64/arm64 manifest. Latest digest: `sha256:6e1678ad9af3514482e60efc7eae16cf9ab80c0b2b22b1eddbd8e0b0eb3a1251`.

## 2026-06-08T06:22:49Z

Simplified the CLI capture workflow with `scripts/quick-probe.mjs` and `npm run probe`. The one-command probe hits `/api/fingerprint`, runs the companion upload, optionally tests `/proxy?target=...` with `--proxy-target`, prints a short JSON summary, and exits. The browser capture panel now presents this one command instead of the separate 1/2/3 flow. Validation passed: `npm test` 10/10, `node --check public/app.js`, `node --check src/server.js`, `node --check src/cpa.js`, `node --check scripts/cli-companion.mjs`, `node --check scripts/quick-probe.mjs`, `docker compose config`, local quick-probe smoke with raw/auth, local quick-probe smoke with proxy target, and pulled-GHCR container quick-probe smoke. Pushed commit `f144c1d` and multi-arch GHCR tags `latest` and `f144c1d`; verified amd64/arm64 manifest. Latest digest: `sha256:0477bf4d3ebc9cd97c5cf875a58f80c06a5e898d595297120ef69500a561494d`.

## 2026-06-08T06:41:46Z

Added no-residue temporary probe launchers. `scripts/temp-probe.sh` and `scripts/temp-probe.ps1` create a temporary directory, download `quick-probe.mjs` and `cli-companion.mjs`, use existing Node 18+ when available, otherwise temporarily download portable Node into the same temporary directory, run the probe, and delete the temporary directory on exit. The browser panel and README now show Linux/macOS and Windows one-liners instead of requiring a checked-out project environment. Validation passed: `npm test` 10/10, `node --check public/app.js`, `node --check src/server.js`, `node --check src/cpa.js`, `node --check scripts/cli-companion.mjs`, `node --check scripts/quick-probe.mjs`, `bash -n scripts/temp-probe.sh`, PowerShell parser check for `scripts/temp-probe.ps1`, `docker compose config`, and Windows raw temp-probe smoke from GitHub raw URL with no leftover `rt-refresh-probe-*` directories. Pushed commit `f84b5bb` and multi-arch GHCR tags `latest` and `f84b5bb`; verified amd64/arm64 manifest. Latest digest: `sha256:b6f008c0f7491fe3065d5d10363ded524510372e72ac609ff2a0a234abfc5219`.
